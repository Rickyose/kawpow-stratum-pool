...
exports.createGeneration = function(rpcData, blockReward, feeReward, recipients, poolAddress){
    ...
    if (rpcData.coinbase_transactions){
        for (var i = 0; i < rpcData.coinbase_transactions[0].vout.length; i++) {
            if (rpcData.coinbase_transactions[0].vout[i].value != blockReward && rpcData.coinbase_transactions[0].vout[i].value != 0){
                tx.addOutput(
                        rpcData.coinbase_transactions[0].vout[i].scriptPubKey,
                        rpcData.coinbase_transactions[0].vout[i].value
                    )
            }
        }
    }
