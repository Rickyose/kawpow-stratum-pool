...
exports.createGeneration = function(rpcData, blockReward, feeReward, recipients, poolAddress){
    ...
    if (rpcData.coinbase_outputs){
        for (var i = 0; i < rpcData.coinbase_outputs.length; i++) {
            var output_address 		= rpcData.coinbase_outputs[i].address;
            var output_address_hash 	= bitcoin.address.fromBase58Check(output_address).hash;
            tx.addOutput(
                        scriptCompile(output_address_hash),
                        rpcData.coinbase_outputs[i].nValue
            )
        }
    }
