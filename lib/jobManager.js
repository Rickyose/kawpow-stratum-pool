...
var JobManager = module.exports = function JobManager(options) {
  ...
  this.processShare = function (miner_given_jobId, previousDifficulty, difficulty, miner_given_nonce, ipAddress, port, workerName, miner_given_header, miner_given_mixhash, extraNonce1, callback_parent) {
    ...
    if (options.kawpow_validator == "kawpowd") {
      ...
      async.series([
        function(callback) {
          var isValid = kawpow.verify(header_hash_buffer, miner_given_nonce.toBuffer(), job.rpcData.height, miner_given_mixhash.toBuffer(), target_share_hex.toBuffer());
          ...
          isValid = kawpow.verify(header_hash_buffer, miner_given_nonce.toBuffer(), job.rpcData.height, miner_given_mixhash.toBuffer(), job.target_hex.toBuffer());
