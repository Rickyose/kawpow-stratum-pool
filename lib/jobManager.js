...
var JobManager = module.exports = function JobManager(options) {
  ...
  this.processShare = function (miner_given_jobId, previousDifficulty, difficulty, miner_given_nonce, ipAddress, port, workerName, miner_given_header, miner_given_mixhash, extraNonce1, callback_parent) {
    ...
    if (options.kawpow_validator == "kawpowd") {
      ...
      async.series([
        function(callback) {
          var isValid = hasher_kawpow.verify(header_hash_buffer, Buffer.from(miner_given_nonce, "hex"), job.rpcData.height, Buffer.from(miner_given_mixhash, "hex"), Buffer.from(target_share_hex, "hex"));

          if (isValid) {
                console.log(target_share_hex);
            }
            else {
                console.log('Invalid');
          }

          isValid = hasher_kawpow.verify(header_hash_buffer, Buffer.from(miner_given_nonce, "hex"), job.rpcData.height, Buffer.from(miner_given_mixhash, "hex"), Buffer.from(job.target_hex, "hex"));

          if (isValid) {
                console.log(job.target_hex);
            }
            else {
                console.log('Invalid');
          }
