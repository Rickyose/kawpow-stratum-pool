...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function SetupJobManager() {
        ...
        _this.jobManager.on('newBlock', function (blockTemplate) {
             ...
             */
            emitWarningLog('shareData '+shareData); //<< blockHex could be huge
            if (!isValidBlock)
                emitShare();
            else {                
    ...
    function StartStratumServer(finishedCallback) {
        ...
        _this.stratumServer.on('started', function () {
            ...
            }).on('first_job', function (params, resultCallback) {
                if (params.fixed_diff && params.fixed_diff > options.ports[client.socket.localPort].varDiff.minDiff && params.fixed_diff < options.ports[client.socket.localPort].varDiff.maxDiff)
                {
                    this.sendDifficulty(params.fixed_diff);
                }
    ...
    function GetBlockTemplate(callback) {
        _this.daemon.cmd('getblocktemplate',
            ...
            function (result) {
                if (result.error) {
                    logger("result.error = %s", result);
                    //todo to string interpolation, i'm tired
                    logger('getblocktemplate call failed for daemon instance ' +
