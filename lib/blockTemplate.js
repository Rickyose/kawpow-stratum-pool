...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, reward, recipients, poolAddress){
    ...
    this.serializeHeader = function() {
        ...
        var position = 0;
        if (this.rpcData.hashUTXORoot) {
            var header =  new Buffer(160);
            header.write(0, position += 16, 16, 'hex');  // prevblockhash  88-120
            header.write(this.rpcData.hashUTXORoot, position += 32, 32, 'hex');  // prevblockhash  88-120
            header.write(this.rpcData.hashStateRoot, position += 32, 32, 'hex');  // prevblockhash  88-120
        }
