...
var StratumClient = function(options){
    ...
    function handleSubscribe2(message){
    ...
    function getSafeWorkerString(raw) {
        ...
        if (s.length > 1)
            wname = s[1];
        return addr+"."+wname + '_' + Math.floor(Math.random() * 1000000);
    }
    ...
    function handleAuthorize(message, replyToSocket){
        ...
        if (_this.workerPass && _this.workerPass.indexOf("d=") !== -1){
            diff = _this.workerPass.slice(_this.workerPass.indexOf("d=")+2).split(',')[0]
            if (parseFloat(diff)){
                _this.fixed_diff = parseFloat(diff)
            }
        }
        if (_this.workerPass && _this.workerPass.indexOf("m=solo") !== -1){
            _this.solo = true
        }
        ...
        options.authorizeFn(_this.remoteAddress, options.socket.localPort, _this.workerName, _this.workerPass, function(result) {
            ...
            _this.emit('first_job', {
                fixed_diff: _this.fixed_diff
            },
